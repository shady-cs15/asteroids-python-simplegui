...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
            x = 64 + self.age*128
            canvas.draw_image(self.image,[x,64],[128,128],self.pos,[128,128])
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
...
def group_group_collide(group_1,group_2):
    temp = len(group_2)
    for x in list(group_1):
        if group_collide(group_2,x):
            group_1.remove(x)
    return temp - len(group_2)
...
def draw(canvas):
    global time, started,score,lives,soundtrack,rock_group
    time += 1
    wtime = (time / 4) % WIDTH
...
def rock_spawner():
    ...
    for x in range(0,2):
        if -40<a_rock.pos[x]-my_ship.pos[x]<40:
            ignore = True
    if len(rock_group)<12 and ignore == False:
